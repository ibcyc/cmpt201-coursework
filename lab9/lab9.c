...
void *handle_client(void *arg) {
  ...
  char buf[BUF_SIZE];
  ssize_t numRead;
  while ((numRead = read(clientSocketFd, buf, BUF_SIZE - 1)) > 0) {
    buf[numRead] = '\0';
    pthread_mutex_lock(&count_mutex);
    total_message_count++;
    int currentMsgCount = total_message_count;
    pthread_mutex_unlock(&count_mutex);
    printf("Msg #%4d; Client ID %d: %s", currentMsgCount, clientId, buf);
    if (buf[numRead - 1] != '\n') {
      printf("\n");
    }
  }
...
int main() {
  ...
  for (;;) {
    ...
    int newClientId = client_id_counter;
    client_id_counter++;
    pthread_mutex_unlock(&client_id_mutex);
    newClient->cfd = cfd;
    newClient->client_id = newClientId;
    printf("New client created! ID %d on socket FD %d\n", newClientId, cfd);
    pthread_t threadID;
    if (pthread_create(&threadID, NULL, handle_client, newClient) != 0) {
