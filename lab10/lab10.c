...
Understanding the Client:
  1. How is the client sending data to the server? What protocol?
    >>> Send data using Socket, and the protocol is TCP

  2. What data is the client sending to the server?
    >>> Sending a series of char array.

Understanding the Server:
  1. Explain the argument that the `run_acceptor` thread is passed as an
argument.
    >>> "run" is boolean, use to control thread
        "list_handle" is pointer to linked list, store client's received message
        "list_lock" is pointer to mutex, ensure thread-safe access

  2. How are received messages stored?
    >>> Store in singly linked list. which is list_handle

  3. What does `main()` do with the received messages?
    >>> main() calls the collect_all(). this will print collected msg and free
the memory

  4. How are threads used in this sample code?
    >>> the main theard is to setup and cleanup. another acceptor thread is to
listen connection, the other worker thread is to read and store msg
concurrently.

Explain the use of non-blocking sockets in this lab.
  * How are sockets made non-blocking?
    >>> Using fcntl() call with 0_NONBLOCK flag

  * What sockets are made non-blocking?
    >>> BOth the server's listening socket and each client's connection socket

 * Why are these sockets made non-bloking? What purpose does it serve?
    >>> Prevent accept() and read() from hanging the thread. This allow the loop
to keep running and can checl the run flag and shut down when requested
*/
...
int main() {
  ...
  uint32_t total_expected = MAX_CLIENTS * NUM_MSG_PER_CLIENT;
  while (true) {
